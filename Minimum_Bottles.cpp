...
void solve() {
    ...
    int waterBottles, capacity;
    cin >> waterBottles >> capacity;
    int arr[waterBottles];
    bool falg = false;
    for(int i=0; i<waterBottles; i++) {
        cin >> arr[i];
    }

    ll max_sum = accumulate(arr, arr + waterBottles, 0);

    ll ans = ceil(max_sum/capacity);

    if(ans == 0 && max_sum != 0) cout << 1 << "\n";
    else if(max_sum == 0) cout << 0 << "\n";
